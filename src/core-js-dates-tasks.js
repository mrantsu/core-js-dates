...
function getNextFriday(date) {
  ...
  const daysInAWeek = 7;
  const daysToAdd =
    currentDayIndex >= fridayIndex
      ? fridayIndex - currentDayIndex + daysInAWeek
      : fridayIndex - currentDayIndex;
  const daysToAddInHours = daysToAdd * 24;
  const daysToAddInHoursToMiliseconds = daysToAddInHours * 3600000;
  const nextFriday = new Date(
    dateObject.getTime() + daysToAddInHoursToMiliseconds
  );
  return nextFriday;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const startDataObj = new Date(dateStart);
  const endDataObj = new Date(dateEnd);
  const subtraction = Math.abs(startDataObj - endDataObj);
  const correctionDay = 1;
  return subtraction / 86400000 + correctionDay;
}
...
function getCountWeekendsInMonth(month, year) {
  const lastDayInAMonth = new Date(year, month, 0);

  let weekendCount = 0;
  const currentDate = new Date(year, month - 1);

  while (currentDate <= lastDayInAMonth) {
...
function getWeekNumberByDate(date) {
  ...
  const firstDayOfYear = new Date(nearestMonday.getFullYear(), 0, 1);
  const millisecondsPassed = nearestMonday - firstDayOfYear;
  const weekNumber = Math.ceil(
    millisecondsPassed / (1000 * 60 * 60 * 24 * 7) + 1
  );
...
function getNextFridayThe13th(date) {
  ...
  while (!nextFridayDate) {
    const day = new Date(currentYear, currentMonth, 13).getDay();
    if (day === 5) {
      nextFridayDate = new Date(currentYear, currentMonth, 13);
    } else {
      currentMonth += 1;
    }
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const startDateData = period.start.split('-').reverse().join('-');
  const endDateData = period.end.split('-').reverse().join('-');
